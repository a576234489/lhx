...
$.fn.ComboBox = function (options) {
    ...
    //操作搜索事件
    if (options.allowSearch) {
        $option.find('.ui-select-option-search').find('input').on("keydown", function (e) {
            if (e.keyCode === 13) {
                var value = $(this).val();
...
$.fn.authorizeColModel = function () {
    ...
    $.each(columnModel, function (i) {
        if (columnModel[i].name !== "rn") {
            $element.hideCol(columnModel[i].name); //隐藏列
        }
    });
    var moduleId = tabiframeId().substr(12);
    ...
    if (typeof data !== 'undefined') {
        $.each(data, function (i) {
            $element.showCol(data[i].EnCode); //显示列
        });
    }
};
...
$.fn.jqGridRowValue = function (n) {
    ...
    var rowData;
    if (typeof selectedRowIds !== 'undefined' && selectedRowIds !== "") {
        var len = selectedRowIds.length;
        for (var i = 0; i < len ; i++) {
            rowData = $jgrid.jqGrid('getRowData', selectedRowIds[i]);
            json.push(rowData[code]);
        }
    } else {
        rowData = $jgrid.jqGrid('getRowData', $jgrid.jqGrid('getGridParam', 'selrow'));
        json.push(rowData[code]);
    }
    return String(json);*/
    var i = $(this),
...
$.isNullOrEmpty = function (obj) {
    if ((typeof obj === "string" && obj === "") || obj == null || typeof obj === 'undefined') {
        return true;
    }
    else {
        return false;
...
checkedArray = function (id) {
    var isOK = true;
    if (typeof id === 'undefined' || id === "" || id === 'null' || id === 'undefined') {
        isOK = false;
...
function tableData(RowData, tableData) {
    if(tableData && !tableData.length) return '';
    var res = {}, i, len = tableData.length;
    for(i=0; i<len; i++){
        res[tableData[i]] = RowData[tableData[i]]
    }
    return res
}
