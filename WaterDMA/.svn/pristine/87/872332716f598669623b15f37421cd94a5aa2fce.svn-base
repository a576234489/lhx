...
<script>
    ...
    function GetGrid() {
        ...
        var queryJson = $("#filter-form").GetWebControls();
        $gridTable.jqGrid({
            url: "../../LeakageQuery/CommLog/GetPageListJson",
            postData: { queryJson: JSON.stringify(queryJson) },
            datatype: "json",
            ...
            colModel: [
                ...
                { name: 'LogType', label: '日志类型', width: 80, align: 'left' },
                { name: 'M_Name', label: '站点名称', width: 150, align: 'left' },
                { name: 'M_PipeDn', label: '管径大小', width: 80, align: 'left' },
                { name: 'LogSource', label: '日志名称', width: 100, align: 'left' },
                { name: "LogTime", label: "记录时间", width: 140, align: "left" },
                { name: 'LogText', label: '日志信息', width: 250, align: 'left' }
            ],
            viewrecords: true,
            rowNum: 20,
            rowList: [10, 20, 30, 50, 100],
            pager: "#gridPager",
            sortname: 'LogTime DESC',
            rownumbers: true,
            shrinkToFit: false,
            gridview: true,
